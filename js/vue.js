var Vue = (function (exports) {
  ...
  function warn(msg, ...args) {
    ...
    if (appWarnHandler) {
      callWithErrorHandling(appWarnHandler, instance, 11 /* ErrorCodes.APP_WARN_HANDLER */, [msg + args.join(''), instance && instance.proxy, trace.map(({ vnode }) => `at <${formatComponentName(instance, vnode.type)}>`).join('\n'), trace])
    } else {
  ...
  */
  ...
  function finishComponentSetup(instance, isSSR, skipOptions) {
    ...
    if (!Component.render && instance.render === NOOP && !isSSR) {
      ...
      if (!compile$1 && Component.template) {
        warn(`Component provided template option but ` + `runtime compilation is not supported in this build of Vue.` + ` Use "vue.global.js" instead.` /* should not happen */)
      } else {
  ...
  const transformFor = createStructuralDirectiveTransform('for', (node, dir, context) => {
    ...
    return processFor(node, dir, context, forNode => {
      ...
      forNode.codegenNode = createVNodeCall(context, helper(FRAGMENT), undefined, renderExp, fragmentFlag + ` /* ${PatchFlagNames[fragmentFlag]} */`, undefined, undefined, true /* isBlock */, !isStableFragment /* disableTracking */, false /* isComponent */, node.loc)
      return () => {
        ...
        if (slotOutlet) {
          ...
          childBlock = createVNodeCall(context, helper(FRAGMENT), keyProperty ? createObjectExpression([keyProperty]) : undefined, node.children, 64 /* PatchFlags.STABLE_FRAGMENT */ + ` /* ${PatchFlagNames[64 /* PatchFlags.STABLE_FRAGMENT */]} */`, undefined, undefined, true, undefined, false /* isComponent */)
        } else {
